...
static void vdpau_update_length(struct ra_swchain *sw, int depth)
{
    struct priv *p = sw->ctx->priv;

    while (depth < p->num_surfaces) {
        // Remove the surface that was used the least recently, which is
        // the next surface we will render to (i.e. p->idx_surfaces)
        destroy_vdpau_surface(sw->ctx, &p->surfaces[p->idx_surfaces]);
        MP_TARRAY_REMOVE_AT(p->surfaces, p->num_surfaces, p->idx_surfaces);
    }

    while (depth > p->num_surfaces) {
        struct surface surf = { .surface = VDP_INVALID_HANDLE };
        MP_TARRAY_APPEND(p, p->surfaces, p->num_surfaces, surf);
    }
}
...
static const struct ra_swchain_fns vdpau_swchain;
...
static bool vdpau_init(struct ra_ctx *ctx)
{
    ...
    p->num_surfaces = ctx->opts.swchain_depth + 1; // +1 for the visible image
    ...
    // Override the swapchain functions with our own
    ctx->swchain->fns = &vdpau_swchain;
...
static const struct ra_swchain_fns vdpau_swchain = {
    ...
    .update_length = vdpau_update_length,
