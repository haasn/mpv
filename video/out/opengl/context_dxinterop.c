                               ...
                               D3DPRESENT_PARAMETERS *pparams)
{
    ...
    *pparams = (D3DPRESENT_PARAMETERS) {
        ...
        .BackBufferCount = ctx->opts.swchain_depth + 2,
...
static int d3d_create(struct ra_ctx *ctx)
{
    ...
    IDirect3DDevice9Ex_SetMaximumFrameLatency(p->device, ctx->opts.swchain_depth);
...
static void resize(struct ra_ctx *ctx)
{
    struct priv *p = ctx->priv;
    dxgl_reset(ctx);
    ra_gl_ctx_resize(ctx->swchain, ctx->vo->dwidth, ctx->vo->dheight, p->main_fb);
}
