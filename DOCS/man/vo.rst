...
``opengl``
    ...
    ``opengl-hq-XXX`` profile to use this driver with defaults set to high
    quality rendering.
...
``opengl-cb``
    ...
    This also supports many of the options the ``opengl`` VO has. Run
...
``--scale=<filter>``
    ...
    ``spline36``
        Mid quality and speed. This is the default when using ``opengl-hq-XXX``.
